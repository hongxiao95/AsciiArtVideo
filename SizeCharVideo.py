...
cap = cv2.VideoCapture("SituMovie2.mp4")
...
for fr in range(sourceFrameCount):
    ...
    for i in range(0, sourceHeight, colF):
        for j in range(0, sourceWidth, colF):
            ...
            cv2.putText(outImgs[fr], '1S',(j * 4, i * 4), cv2.FONT_HERSHEY_SIMPLEX, ftSize * (pixGray / 255), (255,255,255),ftWidth)
...
writer = cv2.VideoWriter("SituWechat2.avi", fcc, sourceFPS, (outWidth, outHeight))
for i in range(sourceFrameCount):
