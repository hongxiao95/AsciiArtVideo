...
cap = cv2.VideoCapture("SituMovie4.mp4")
...
for fr in range(sourceFrameCount):
    ...
    for i in range(0, sourceHeight, colF):
        for j in range(0, sourceWidth, colF):
            cv2.putText(outImgs[fr], charSetGray[int((sum(srcArray[i][j]) / 3) / 17)],(j * 4, i * 4), cv2.FONT_HERSHEY_SIMPLEX, ftSize, (int(srcArray[i][j][0]), int(srcArray[i][j][1]), int(srcArray[i][j][2])), ftWidth)
...
writer = cv2.VideoWriter("SituWechat4.avi", fcc, sourceFPS, (outWidth, outHeight))
for i in range(sourceFrameCount):
